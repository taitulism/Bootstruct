...
module.exports = {
	...
	entry_handlers (entryMap) {
		...
		if (entryMap.type === FILE) {
			simpleHookFile('entry_handlers', entryMap.path, extending, MUST_BE_A_FUNCTION);
		}
		else {
			simpleHookFolder('entry_handlers', entryMap.entries, extending);
		}
	},
	...
	shared_methods (entryMap) {
		...
		if (entryMap.type === FILE) {
			this.shared_methods = simpleHookFile('entry_handlers', entryMap.path, extending, MUST_BE_A_FUNCTION);
		}
		else {
			this.shared_methods = simpleHookFolder('entry_handlers', entryMap.entries, extending);
		}
	},
