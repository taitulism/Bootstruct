...
var entryHandlers = {
	...
	entry_handlers: function (entryMap) {
		...
		if (handlersObj) {
			forIn(handlersObj, function (key, val) {
				var entryHandler;

				key = normalizeEntryName(key);

				if (typeof val == 'function') { // file
					trySetEntryHandler(key, val);
				}
				else if (val.path){ // folder. "val" is an entryMap
					entryHandler = tryReqFn(val.path);

					if (entryHandler) {
						trySetEntryHandler(key, entryHandler);
					}
				}
				else {
					console.log('Bootstruct Error:');
					console.log('   entryHandler "' + key + '" should be a function.');
				}
			});
		}
	},
...
function trySetEntryHandler (name, method) {
	if (!webRootHandlers[name]) {
		webRootHandlers[name] = method;
	}
	else {
		console.log('Bootstruct Error:');
		console.log('   You are overriding entryHandlers\' "' + name + '" prop/method.');
		console.log('   Use another name.');
	}
}
