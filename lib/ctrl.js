...
function shouldBeIgnored (app, entry) {
	if ( matchStartWith(app, entry) ) return true;

	if ( isInIgnoreList(app, entry) ) return true;
...
module.exports = function () {
	...
	Ctrl.prototype = { constructor: Ctrl,
		...
		*/
		parseFolderMap: function (folderMap) {
			var self     = this;
			...
			forIn(folderMap.entries, function (key, entryMap) {
				...
				/* Deal Breaker */ if ( shouldBeIgnored(global, name) ) return;

				isFile = entryMap.type;

				// if such entryHandler exists
				if (handlers[name]) {
					handlers[name].call(self, entryMap);
				}
				else {
					if (isFile) {
	    ...
	    */
		checkOut: function (io) {
			if (this.name === 'RC') { // RC ends response
				io.res.end();
			}
			else {
				this.parent.next(io);
