...
function shouldBeIgnored (entry) {
	if (ignoreFirstChar(this, entry)) return true;
...
function ignoreFirstChar (name) {
	var char_, i, len;

	if (typeof ignoredFirstChars == 'string') {
		if (name[0] === ignoredFirstChars) {
			return true;
		}
		return false;
	}
	else if (Array.isArray(ignoredFirstChars)) {
		len = ignoredFirstChars.length;

		for (i = 0; i < len; i+=1) {
			char_ = ignoredFirstChars[i];
			
			if (typeof char_ == 'string') {
				if (name[0] === char_) {
					return true;
				}
			}
...
var has = {
	...
	methods: function hasMethods (ctrl) {
		...
		if (ctrl.app._shared_methods) {
			sharedMethods = (Object.keys (ctrl.app._shared_methods).length > 0);
		}
	...
	subCtrls: function hasSubCtrls (ctrl) {
		...
		if (ctrl.app._shared_ctrls) {
			sharedCtrls = (Object.keys (ctrl.app._shared_ctrls).length > 0);
		}
...
// ==========================================
function Ctrl (folderMap, name, parent, app, isRC) {
	// meta
	...
	this.methods  = Object.create(this.app._shared_methods || null);
	this.subCtrls = Object.create(this.app._shared_ctrls   || null);
