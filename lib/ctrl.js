...
const normalizeEntryName = require('./utils/normalize-entry-name');
const shouldBeIgnored    = require('./utils/should-be-ignored');
...
const has = {
	...
	methods (ctrl) {
		...
		const ownMethods = (Object.keys(ctrl.methods).length > 0);
	...
	subCtrls (ctrl) {
		...
		const ownCtrls = Object.keys(ctrl.subCtrls).length > 0;
...
const run = {
	...
	method (io) {
		const next   = io.params[0];
		const method = this.methods[next.toLowerCase()];
	...
	subCtrl (io) {
		const next = io.params[0];
		const subCtrl = this.subCtrls[next.toLowerCase()];
