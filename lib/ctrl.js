...
var webRootHandlers    = require('./entryHandlers/ctrl');
...
function addToIgnoreList (item) {
	...
	ignoreListLen = ignoreList.push(item);
}
...
function shouldBeIgnored (entry) {
	...
	if ( isInIgnoreList(entry) ) return true;
...
function setID (ctrl) {
	...
	// is RC? id = '/'
	if (ctrl.name === 'RC') return '/';

	// RC's direct child - avoid double slashes
	if (ctrl.parent && ctrl.parent.name !== 'RC') buildID += ctrl.parent.id;
...
function parseFolderMap (ctrl, folderMap) {
	forIn(folderMap.entries, function (key, entryMap) {
		...
		/* Deal Breaker */ if ( shouldBeIgnored(name) ) return;
...
function initSubCtrls (ctrl) {
	...
	forIn(subCtrls, function (name, subCtrlMap) {
		...
		subCtrls[name] = new Ctrl(name, subCtrlMap, ctrl);
	});
}
...
CtrlProto.name      = null;
CtrlProto.folderMap = null;
CtrlProto.parent    = null;
CtrlProto.global    = null;
CtrlProto.id        = null;
CtrlProto.verbs     = null;
CtrlProto.methods   = null;
CtrlProto.subCtrls  = null;
CtrlProto.chains    = null;
...
module.exports.addToIgnoreList = addToIgnoreList;