...
function debugNext (ctrl, io, chain, i) {
	...
	const reqVerb        = io.req.method.toLowerCase();
	...
	else if (nextName && nextName === 'verb' && ctrl.verbs[reqVerb].path) {
		console.log('   ', ctrl.verbs[reqVerb].path.substr(webrootLenDiff));
	}
}
