...
function debugNext (ctrl, io, nextInChain) {
	...
	if (nextInChain.path) {
		...
		if (nextName === 'verb' && nextVerb) {
			...
			const methodPath = getSharedMethodPath(methodName, ctrl, app);
...
function getSharedMethodPath (methodName, ctrl, app) {
	const path = ctrl.methods[methodName].path;

	if (path) return path;

	return resolvePath(app.hooksFolderPath, 'shared_methods', methodName);
}
