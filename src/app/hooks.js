...
module.exports = {
	...
	io_init (entryMap) {
		...
		const {content, err} = getFn(entryMap.path);
		...
		if (content) {
			...
			this.IO.prototype.init = content;
		}
		else {
			throw error.ioInitExpectedAFunction(err);
		}
	},
	...
	io_exit (entryMap) {
		...
		if (content) {
			this.IO.prototype.exit = content;
		}
		else {
			throw error.ioExitExpectedAFunction(err);
		}
	},
